...
$('settingsForm').onsubmit=e=>{ e.preventDefault(); saved.printerIp=$('printerIp').value.trim(); saved.mainboardId=$('mainboardId').value.trim(); saved.cameraUrl=$('cameraUrl').value.trim(); localStorage.setItem('dashboard',JSON.stringify(saved)); $('settingsDialog').close(); stopConnection(); connect(); };
